...
namespace WedService
{
    ...
    public class UserWed : IUserWed
    {
        ...
        {
            var sp = (from p in db.SanPhams where p.TinhTrang != "Ẩn" select p).ToList();
            return sp;
        }
        ...
        {
            var sp = (from p in db.SanPhams where p.MaLoai == maloai && p.TinhTrang != "Ẩn" orderby p.Gia select p).ToList();
            return sp;
        }
        public List<SanPham> FindAllSPKhuyenMai()
        {
            var sp = (from p in db.SanPhams where p.GiaGiam > 0 && p.TinhTrang != "Ẩn" orderby p.MaSP select p).ToList();
            return sp;
        }
        public SanPham FindSanPham(string masp)
        {
            var tim = (from p in db.SanPhams where p.MaSP == masp && p.TinhTrang != "Ẩn" select p).Single();
            return tim;
        }
        ...
        public List<SanPham> FindTimKiemCB(string timkiem)
        {
            ...
            {
                sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.TinhTrang != "Ẩn" orderby p.Gia select p).ToList();
            }
            return sp;
        }
            ...
            if (!String.IsNullOrEmpty(timkiem))
            {
                if(phanloai == "-1")
                {
                    if(giatu == 0)
                    {
                        if(giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                    else
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && ((p.Gia - p.GiaGiam) >= giatu) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && ((p.Gia - p.GiaGiam) >= giatu) && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                }
                else
                {
                    if (giatu == 0)
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.MaLoai == phanloai && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                    else
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) >= giatu) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.TenSP.Contains(timkiem) && p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) >= giatu) && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                }
            }
            else
            {
                if (phanloai == "-1")
                {
                    if (giatu == 0)
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                    else
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where ((p.Gia - p.GiaGiam) >= giatu) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where ((p.Gia - p.GiaGiam) >= giatu) && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                }
                else
                {
                    if (giatu == 0)
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.MaLoai == phanloai && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                    else
                    {
                        if (giaden == 0)
                        {
                            sp = (from p in db.SanPhams where p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) >= giatu) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                        else
                        {
                            sp = (from p in db.SanPhams where p.MaLoai == phanloai && ((p.Gia - p.GiaGiam) >= giatu) && ((p.Gia - p.GiaGiam) <= giaden) && p.TinhTrang != "Ẩn" select p).ToList();
                        }
                    }
                }
            }
